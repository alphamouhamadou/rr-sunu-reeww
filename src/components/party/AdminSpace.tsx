...
export function AdminSpace() {
  const { adminSection, setAdminSection, isAdminAuthenticated, setIsAdminAuthenticated, setCurrentSection } = useAppStore()
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.member && data.member.role === 'admin') {
        ...
        setAdminLoginError('Accès non autorisé. Seuls les administrateurs peuvent accéder à cette section.')
      }
    } catch {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center px-4">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <div className="w-20 h-20 rounded-full overflow-hidden party-gradient flex items-center justify-center mx-auto mb-4">
              <Shield className="w-10 h-10 text-white" />
            </div>
            <h1 className="text-2xl font-bold text-[#008751]">Administration</h1>
            <p className="text-gray-500 mt-2">Renaissance Républicaine Sunu Reew</p>
          </div>
          <Card>
            <CardHeader>
              ...
              <form onSubmit={handleAdminLogin} className="space-y-4">
                {adminLoginError && (
                  <Alert className="bg-[#CE1126]/10 border-[#CE1126]">
                    <AlertDescription className="text-[#CE1126]">{adminLoginError}</AlertDescription>
                  </Alert>
                )}
                <div>
